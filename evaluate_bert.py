...
from process import load_model, load_msr, load_msr_bert
import bert_code
...
def collect_bert(model, tokenizer, w2v):
    '''
        Collects matrix and vocabulary list from a trained model.
        Helper function. You shouldn't have to call this yourself.
    '''
    ...
    matrix = []
    for w in vocab:
        matrix.append(bert_code.get_embeddings_from_text(w, tokenizer, model))
    return np.array(matrix), vocab, indices
...
def eval_wordsim_bert(model, tokenizer, f='data/wordsim353/combined.tab'):
    '''
        Evaluates a trained embedding model on WordSim353 using cosine
        similarity and Spearman's rho. Returns a tuple containing
        (correlation, p-value).
    ...
    for line in open(f, 'r').readlines():
        ...
        tokenized_text1, token_tensor1, segment_tensor1 = bert_code.get_bert_inputs(w1, tokenizer)
        v1 = bert_code.get_bert_embeddings(token_tensor1, segment_tensor1, model).squeeze().numpy()

        tokenized_text2, token_tensor2, segment_tensor2 = bert_code.get_bert_inputs(w2, tokenizer)
        v2 = bert_code.get_bert_embeddings(token_tensor2, segment_tensor2, model).squeeze().numpy()
...
def eval_bats_file(model, matrix, vocab, indices, f, tokenizer, repeat=False,
                   multi=0):
    '''
    ...
    if not multi:
        ...
        for i in range(len(pairs)):
            ...
            if j >= i: j += 1
            a = bert_code.get_cls_embeddings_from_text(pairs[i][0], tokenizer, model)
            c = bert_code.get_cls_embeddings_from_text(pairs[j][0], tokenizer, model)
            for bw in pairs[i][1]:
                qa.append(a)
                qb.append(bert_code.get_cls_embeddings_from_text(bw, tokenizer, model))
                qc.append(c)
                groups.append(i)
...
def eval_bats(model, matrix, vocab, indices, tokenizer):
    '''
    ...
    for dr in os.listdir('data/BATS'):
        if os.path.isdir(os.path.join(base, dr)):
            ...
            for f in os.listdir(os.path.join(base, dr)):
                accs[f.split('.')[0]] = eval_bats_file(model, matrix, vocab, indices, os.path.join(base, dr, f), tokenizer)
...
def eval_msr_bert(tokenizer, model):
    '''
        Evaluates a trained embedding model on the MSR paraphrase task using
        logistic regression over cosine similarity scores.
    '''
    X_tr, y_tr = load_msr_bert('data/msr/msr_paraphrase_train.txt')
    X_test, y_test = load_msr_bert('data/msr/msr_paraphrase_test.txt')
    print(X_tr[0])


    train = [[bert_code.get_cls_embeddings_from_text(ss[0], tokenizer, model),
              bert_code.get_cls_embeddings_from_text(ss[1], tokenizer, model)] for ss in X_tr]
    test = [[bert_code.get_cls_embeddings_from_text(ss[0], tokenizer, model),
              bert_code.get_cls_embeddings_from_text(ss[1], tokenizer, model)] for ss in X_test]
    """
    train = [[np.sum([bert_code.get_embeddings_from_text(w, tokenizer, model) for w in ss[0]], axis=0),
              np.sum([bert_code.get_embeddings_from_text(w, tokenizer, model) for w in ss[1]], axis=0)] for ss in X_tr]
    test = [[np.sum([bert_code.get_embeddings_from_text(w, tokenizer, model) for w in ss[0]], axis=0),
             np.sum([bert_code.get_embeddings_from_text(w, tokenizer, model) for w in ss[1]], axis=0)] for ss in X_test]
    """
...
def eval_msr(model):
    ...
    train = [[np.sum([model[w] for w in ss[0] if w in model], axis=0),
              np.sum([model[w] for w in ss[1] if w in model], axis=0)] for ss in X_tr]
    test = [[np.sum([model[w] for w in ss[0] if w in model], axis=0),
             np.sum([model[w] for w in ss[1] if w in model], axis=0)] for ss in X_test]
...
if __name__ == "__main__":
    ...
    print('[evaluate] Loading model for vocab...')
    w2v_model = load_model("word2vec_for_bert.wordvectors")
    #matrix, vocab, indices = collect(w2v_model)

    bert_model, tokenizer = bert_code.initialize_models()

    print('[evaluate] Collecting matrix...')
    matrix, vocab, indices = collect_bert(bert_model, tokenizer, w2v_model)


    print('[evaluate] WordSim353 correlation:')
    ws = eval_wordsim_bert(bert_model, tokenizer)
    print(ws)


    print('[evaluate] BATS accuracies:')
    bats = eval_bats(bert_model, matrix, vocab, indices, tokenizer)
    print(bats)


    print('[evaluate] MSR accuracy:')
    msr = eval_msr_bert(tokenizer, bert_model)
    print(msr)
